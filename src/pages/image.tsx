...
import AtomImageSplit from '@Src/components/@atoms/AtomSplitImage';
...
const PageIndex: NextPageFC = () => {
    ...
    async (_, croppedAreaPixels) => {
      try {
        if (file) {
          ...
          const croppedImag = await getCroppedImg(file, croppedAreaPixels, 0);
          ref.current?.setAttribute('data-src-original', croppedImage);
          setCroppedImage(croppedImage);
          console.log(`mire `, croppedImag);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  return (
    <AtomPage>
      ...
      <AtomWrapper>{bandera && <AtomImageSplit image={file} />}</AtomWrapper>
    </AtomPage>
  );
};
